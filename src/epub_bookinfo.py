...
# ==================== 获取信息结束 ====================
    ...
    # epub模式需要获取作者名
    author_name = soup.find('div', {'class': 'sub-title'}).find('a').text.strip()

    # 关闭Browser实例
    await browser.close()

    return {'intro': intro, 'title': title, 'info': info, 'chapters': chapters, 'author': author_name}
